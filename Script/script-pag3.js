function func_popup1() {
    document.getElementById('popup1').style.display = 'block'
    document.getElementById('mais1').style.display = 'none'
}
function func_popup2() {
    document.getElementById('popup2').style.display = 'block'
    document.getElementById('mais2').style.display = 'none'
}
function func_popup3() {
    document.getElementById('popup3').style.display = 'block'
    document.getElementById('mais3').style.display = 'none'
}
function func_popup4() {
    document.getElementById('popup4').style.display = 'block'
    document.getElementById('mais4').style.display = 'none'
}
function func_popup5() {
    document.getElementById('popup5').style.display = 'block'
    document.getElementById('mais5').style.display = 'none'
}
function func_popup6() {
    document.getElementById('popup6').style.display = 'block'
    document.getElementById('mais6').style.display = 'none'
}
function func_popup7() {
    document.getElementById('popup7').style.display = 'block'
    document.getElementById('mais7').style.display = 'none'
}
function func_popup8() {
    document.getElementById('popup8').style.display = 'block'
    document.getElementById('mais8').style.display = 'none'
}
function func_popup9() {
    document.getElementById('popup9').style.display = 'block'
    document.getElementById('mais9').style.display = 'none'
}
function func_popup10() {
    document.getElementById('popup10').style.display = 'block'
    document.getElementById('mais10').style.display = 'none'
}






function func_fec1() {
    document.getElementById('popup1').style.display = 'none'
    document.getElementById('mais1').style.display = 'block'
}
function func_fec2() {
    document.getElementById('popup2').style.display = 'none'
    document.getElementById('mais2').style.display = 'block'
}
function func_fec3() {
    document.getElementById('popup3').style.display = 'none'
    document.getElementById('mais3').style.display = 'block'
}
function func_fec4() {
    document.getElementById('popup4').style.display = 'none'
    document.getElementById('mais4').style.display = 'block'
}
function func_fec5() {
    document.getElementById('popup5').style.display = 'none'
    document.getElementById('mais5').style.display = 'block'
}
function func_fec6() {
    document.getElementById('popup6').style.display = 'none'
    document.getElementById('mais6').style.display = 'block'
}
function func_fec7() {
    document.getElementById('popup7').style.display = 'none'
    document.getElementById('mais7').style.display = 'block'
}
function func_fec8() {
    document.getElementById('popup8').style.display = 'none'
    document.getElementById('mais8').style.display = 'block'
}
function func_fec9() {
    document.getElementById('popup9').style.display = 'none'
    document.getElementById('mais9').style.display = 'block'
}
function func_fec10() {
    document.getElementById('popup10').style.display = 'none'
    document.getElementById('mais10').style.display = 'block'
}